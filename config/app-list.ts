// src/config/app-list.ts

// 这是一个映射表，把 AI 可能会说出来的名字映射到包名
export const APP_PACKAGES: Record<string, string> = {
  ...
  "B站": "tv.danmaku.bili", // Added for compatibility with existing web list
  "哔哩哔哩": "tv.danmaku.bili", // Added for compatibility with existing web list
  ...
  "支付宝": "com.eg.android.AlipayGphone", // Kept from existing web list as it's common
  ...
  // System & Utils (English/International)
  ...
  // Existing System (Chinese) - Preserving some from original web file that might be useful
...
/**
 * 辅助函数：模糊匹配
 * 防止 AI 输出 "打开 微信APP" 这种带后缀的名字导致匹配失败
 */
export function findPackageName(appName: string): string | null {
  // 1. 精确匹配
  if (APP_PACKAGES[appName]) return APP_PACKAGES[appName];

  // 2. 模糊匹配 (简单的包含关系)
  for (const [key, pkg] of Object.entries(APP_PACKAGES)) {
